...
Tips
	...
	quick alias
		...
		ls ~d
	alias for previous working dir
		~+

Docs
	http://zsh.sunsite.dk/Guide/
	http://www.acm.uiuc.edu/workshops/zsh/toc.html - good
...
cd old new
	change old to new in previous path & cd to it

Using dirs/pushd/popd
http://www.acm.uiuc.edu/workshops/zsh/dir_stack.html
	~num	expand to dir at #num in stack
	~+/-num	expand to dir up or down stack
...
Auto Which - invokes which on command
	cmd-name<ESC>?

	Mail checking
		Use mailpath not MAILPATH
	$mailpath=(path-to-check?Message
				path-to-check?Message)
	$MAILCHECK=n seconds (default 60)

Renaming files
	zmv 
		Put "autoload -U zmv" in your ~/.zshrc and
		To remove the extension to all the *.sh files in current
		directory:
		zmv '(*).sh' '$1'
		zmv '(*).c' '$1.x'
		zmv -W '*.c' '*.x'
	...
	Change all from Upper to lower case
	autoload zmv
	zmv '(*)' '${(L)1}'
	change filename extentions
		zmv '(*).lis' '$1.txt'
